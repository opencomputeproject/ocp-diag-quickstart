...
def stress_system_with_binary(step: tv.TestStep) -> None:
    ...
    step.add_measurement(name="Initial temperature", value=system_state["temperature"], unit="C")
    if system_state["temperature"] > 30:
        step.add_error(
            symptom="high-temperature",
            message="Initial temperature is too high to proceed with test",
        )
        step.add_diagnosis(diagnosis_type=tv.DiagnosisType.FAIL, verdict="high-initial-temperature")
        return
    else:
        step.add_log(severity=tv.LogSeverity.INFO, message="Temperature is within limits")
...
def main():
    ...
    with run.scope(dut=tv.Dut(id=socket.gethostname())):
        if not get_system_state().get("temperature"):
            ...
            raise tv.TestRunError(status=tv.TestStatus.SKIP, result=tv.TestResult.NOT_APPLICABLE)
